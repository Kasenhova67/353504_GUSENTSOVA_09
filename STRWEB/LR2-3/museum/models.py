...
def create_default_slider_settings(sender, **kwargs):
    if sender.name == 'museum':
        if not SliderSettings.objects.exists():
            ...
            print("Созданы настройки слайдера по умолчанию")
...
class SliderSettings(models.Model):
    ...
    stop_mouse_hover = models.BooleanField(default=True)
    ...
    def __str__(self):
        return f"Настройки слайдера ({self.updated_at})"
    
    def save(self, *args, **kwargs):
        # Убедимся, что stop_mouse_hover работает только при auto=true
        if not self.auto:
            self.stop_mouse_hover = False
