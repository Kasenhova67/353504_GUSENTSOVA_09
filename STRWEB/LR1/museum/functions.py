...
def get_most_popular_exhibition():
    ...
    valid_orders = [order for order in orders if order.exhibitions is not None]
    
    if not valid_orders:
        return "Нет заказов с выставками"
    
    exhibition_counts = Counter(order.exhibitions.name for order in valid_orders)
    
    if exhibition_counts:
        most_common_exhibition = exhibition_counts.most_common(1)[0][0]
        return most_common_exhibition
    else:
